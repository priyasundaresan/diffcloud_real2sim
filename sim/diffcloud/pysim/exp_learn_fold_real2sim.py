...
with open('conf/rigidcloth/lift/demo_fold.json','r') as f:
	config = json.load(f)
...
def get_partial_cloth_mesh_from_sim(sim):
    face_indices = [1,2,8,9,10,11,12,13,16,19,22,23,24,25,26,27,28,29,30,34,35,37,38,44,45,46,47,48,49,52,55,58,59,60,61,62,63,64,65,66,70,71]
    faces_subset = []
    for i,f in enumerate(sim.cloths[0].mesh.faces):
        if i in face_indices:
            faces_subset.append(f)

    cloth_verts = torch.stack([v.node.x for v in sim.cloths[0].mesh.verts]).float().to(device)
    cloth_faces = torch.Tensor([[vert.index for vert in f.v] for f in faces_subset]).to(device)
    all_verts = [cloth_verts]
    all_faces = [cloth_faces]
    mesh = Meshes(verts=[torch.cat(all_verts)], faces=[torch.cat(all_faces)])
    return mesh
...
def get_loss_per_iter(sim, epoch, sim_step, demo_dir, save, initial_states=None):
    ...
    curr_mesh_partial = get_partial_cloth_mesh_from_sim(sim)
    curr_pcl = sample_points_from_meshes(curr_mesh, num_points)
    curr_pcl_partial = sample_points_from_meshes(curr_mesh_partial, num_points)
    ref_pcl = get_ref_pcl(sim_step, demo_dir)

    #loss_chamfer, _ = chamfer_distance(curr_pcl_cloth_only, ref_pcl) # PRIYA WORKS BEFORE
    loss_chamfer, _ = chamfer_distance_one_sided(curr_pcl_partial, ref_pcl)
    if (save):
        initial_mesh = initial_states[sim_step]
        initial_pcl = sample_points_from_meshes(initial_mesh, num_points)
        #plot_pointclouds([curr_pcl, ref_pcl, initial_pcl], title='%s/fixed_epoch%02d-%03d'%(out_path,epoch,sim_step), angle=50)
        plot_pointclouds([curr_pcl, ref_pcl, initial_pcl], title='%s/fixed_epoch%02d-%03d'%(out_path,epoch,sim_step), angle=160)
        ...
        #plot_pointclouds([curr_pcl_partial, ref_pcl, initial_pcl], title='%s/fixed_epoch%02d-%03d'%(out_path,epoch,sim_step))
        #plot_pointclouds([curr_pcl_partial, ref_pcl, initial_pcl], title='%s/rot_epoch%02d-%03d'%(out_path,epoch,sim_step), angle=(sim_step/total_steps)*95)
...
if __name__ == '__main__':
    ...
    #parser.add_argument('-d', '--demo_dir', type=str, default=os.path.join('fold_real_pcls_gates/fold_polka2'))
    parser.add_argument('-d', '--demo_dir', type=str, default=os.path.join('fold_real_pcls_gates/fold_redblack'))
